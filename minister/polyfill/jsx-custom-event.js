 ...
 */
const React = require('react'); // 定义合法的 lifecycles
...
function jsxCustomEvent(type, props, ...children) {
  ...
  const newProps = Object.assign({}, props); // 包装回调ref， ref 会被调用当 create, update, unmount 
  // element 是一个dom对象
    ...
    if (element) {
      ...
      for (const key in props) {
        ...
        if (Object.prototype.hasOwnProperty.call(props, key) && eventLifeCycles.includes(key.toLowerCase()) && typeof props[key] === 'function' && (!element[key] || element[key] !== props[key])) {
          ...
          element.addEventListener(eventName, props[key], false); // 将props属性全部挂载到dom对象属性上
