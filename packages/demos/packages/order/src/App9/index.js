...
const routerStack = (sessionStorage.getItem('ROUTER_STACK_ORDER') || '').split(',').filter(Boolean)
...
const getClassName = location => {
  ...
  sessionStorage.setItem('ROUTER_STACK_ORDER', routerStack.join()) // 更改后随时保存
  return className
}
...
console.log("window.__MINI_APP_BASE_ROUTE__:", window.__MINI_APP_BASE_ROUTE__);
export default () => <HashRouter basename={window.__MINI_APP_BASE_ROUTE__ || "/"}>
